    ...
    @FXML protected void calculateCompresibility(ActionEvent event){
       ...
       HashMap<Component,Double> fractions = UserProperties.getFractions();
        ...
        if(eos.isCubic()){
            ...
            if(oneRoot.get()){
                double z  = cubiceos.getCompresibilityFactor(pressure, temperature,components,fractions,null,k);
                liquidzTextField.setText(String.valueOf(z));
                
                double vol = cubiceos.getVolume(temperature, pressure, components, fractions, null,k);
                liquidvTextField.setText(String.valueOf(vol));
                
                double fug = cubiceos.getFugacity(pressure, temperature, components, selectedComponent,fractions, null,k);
                liquidFugTextField.setText(String.valueOf(fug));
            }else{
               double liquidz  = cubiceos.getCompresibilityFactor(pressure, temperature,components,fractions,Phase.LIQUID,k);
               double vaporz =  cubiceos.getCompresibilityFactor(pressure, temperature,components,fractions,Phase.VAPOR,k);
               liquidzTextField.setText(String.valueOf(liquidz));
               vaporzTextField.setText(String.valueOf(vaporz));
               
               double liquidVol = cubiceos.getVolume(temperature, pressure, components, fractions, Phase.LIQUID,k);
                liquidvTextField.setText(String.valueOf(liquidVol));
                double vaporVol = cubiceos.getVolume(temperature, pressure, components, fractions, Phase.VAPOR,k);
                vaporvTextField.setText(String.valueOf(vaporVol));            
               
                double liquidFug = cubiceos.getFugacity(pressure, temperature, components, selectedComponent,fractions, Phase.LIQUID,k);
                liquidFugTextField.setText(String.valueOf(liquidFug));
               
                double vaporFug = cubiceos.getFugacity(pressure, temperature, components, selectedComponent,fractions, Phase.VAPOR,k);
